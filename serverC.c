...
void sendmessage()
{
    ...
    remote_addr.sin_port=htons(25651); //服务器端口号
...
//    printf("connected to server \r\n");
    ...
    // 获取子socket的port！！！！！！！！
    getsockname(client_sockfd, (struct sockaddr *) &remote_addr, (socklen_t *) &length);
    //printf("TCP port number %d ",remote_addr.sin_port);
    ...
    printf("The Server C finishe sending its neighbor information to the Client with TCP port number: %d and IP address: %s \n", TCP, inet_ntop(hptr->h_addrtype, *pptr, str, sizeof(str)));
    
    printf("For this connection with Client, the Server C has TCP port number: %d and IP address: %s \n", remote_addr.sin_port,inet_ntop(hptr->h_addrtype, *pptr, str, sizeof(str)));
